...
(async () => {
    try {
        let userName = prompt('Enter your Λbstract name').trim();
        logUser(userName);
        ...
        const now = new Date();
        const expiryDate = new Date(verification.expiry);
        if (expiryDate <= now) {
            alert('Your subscription has expired.');
            return;
        }

        // Calculate and display subscription time remaining
        const timeRemaining = expiryDate - now;
        const tenYearsInMs = 10 * 365 * 24 * 60 * 60 * 1000;
        let expiryMessage;
        if (timeRemaining > tenYearsInMs) {
            expiryMessage = 'Lifetime';
        } else {
            const days = Math.floor(timeRemaining / (1000 * 60 * 60 * 24));
            const hours = Math.floor((timeRemaining % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
            if (days >= 7) {
                expiryMessage = `${days} day${days !== 1 ? 's' : ''}`;
            } else {
                expiryMessage = `${days} day${days !== 1 ? 's' : ''} and ${hours} hour${hours !== 1 ? 's' : ''}`;
            }
        }
        ...
        async function automationLoop() {
            try {
                ...
                await sleep(1000);
                automationLoop(); // Continue the loop forever
            }
        }
        automationLoop();

    } catch (error) {
        alert('Failed to verify name. Please try again later.');
